...
class LinearPDEDataHandler():
	...
	def load_data(self, verbose = 0, cross_validation_ratio = 0, unroll=False, **kwargs):
		...
		x_sample = np.linspace(x - boundaries, x + boundaries, n * 2)
		self._X_train, self._y_train = np.reshape(x_sample, [x_sample.shape[0], 1]), np.zeros([n*2, 1])
